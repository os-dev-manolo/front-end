...
const isValidCNPJ = (cnpj: string): boolean => {
  ...
  const calculateCheckDigit = (digits: string, multipliers: number[]): number => {
    const sum = digits
      .split('')
  ...
  const firstCheckDigit = calculateCheckDigit(base, [5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2]);
...
const schema = Yup.object().shape({
    ...
    cnpj: Yup.string().required("CNPJ é obrigatório").test("is-valid-cnpj", "CNPJ inválido", (value) => value ? isValidCNPJ(value) : false),
